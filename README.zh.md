...
| 覆盖范围 | 行数 |
|---|---:|
| 全部采用的套餐 × 模型点 | <!-- stat:points_total -->264<!-- /stat --> |
| 有月额度的订阅点 | <!-- stat:points_allowance -->244<!-- /stat --> |
...
<!-- stat:points_allowance -->244<!-- /stat --> 个订阅套餐 × 模型点按采用数据里的美元月费拆成三档，避免 GitHub 首页一张图挤满：**$0–30（含 $30）**、**>$30 且 ≤$100**、**>$100–$300**。各档内部按月可用 token 排序。未拆档的全量图和混合比例图仍在 [图表目录](charts/README.md)。
...
把全部 <!-- stat:points_priced -->263<!-- /stat --> 个订阅和 API 点放在同一套 $/MTok 口径下比较。
