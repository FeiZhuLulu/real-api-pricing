...
- **Grok 4.7 服务模型换装**：Cursor Ultra/Fast/Pro/Pro+ 服务模型 grok-4.6→grok-4.7（9/21 起新旗舰）；grok-4.6 按 4.5 先例转派生备选行（1.0 倍率、仍可选同价）。SuperGrok 采用独立双行结构（4.6 旧池行保留 + 4.7 新池实测行，见上条），不再换装。OpenCode Go、GOAT、xAI API 各加 grok-4.7 行（与 4.6 同价 $0.5/$2/$6 ≤200K）。证据：`mimo-v26-grok47-catalogs-round1-2026-09-22.json`、`list-prices-mimo-v26-grok47-round1-2026-09-22.json`。
- **MiMo Token Plan（新套餐点，16 行）**：Lite/Standard/Pro/Max 月池 4.1B/11B/38B/82B Credits ÷ 统一标准负载混合 burn 率折 token——计费本体是共享 Credits 池 + 分模型分类型 burn 率（v2.5-pro cached/input/output=2.5/300/600，v2.5=2/100/200 credits/token），不是固定美元面值额度。¥ 国内价与 $ 国际价同档同池，按 Kimi 先例国内外并点、$ 标价展示（¥39/99/329/659 记入注）。夜间 00:00–08:00（北京）consumption×0.8 → 日/夜两情景点（GLM/DeepSeek 闲时同型）。V2.6 未列入官方套餐支持清单（仅 v2.5 系列 6 模型，ASR 按时长、TTS 免费不画）；耗尽即停不透支；首购/年付 88 折不采。证据：`mimo-token-plan-round1-2026-09-22.json`。
...
- **审计**：采用 202→244 行（订阅 226：225 有额度 + 1 不计额度；按量 18）。plot_quotas.py 补 Xiaomi vendor 映射/颜色/代号（#FFA000）。
