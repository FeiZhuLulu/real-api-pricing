...
当前采用数据共244条“套餐 × 模型”：226条订阅（225条有月额度，另1条不计额度促销点 Devin Pro × SWE-2，≈$0/MTok，促销至2026-10-31）、18条按量API，包括OpenCode Go 30个模型、Command Code GOAT 41个模型、Ollama Pro/Max各11个模型、Step Plan 国内站12个点、MiMo Token Plan 16个点、Devin Max 1个点、ChatGPT Pro 20x Astra 1个点。所有采用数据都参与对应的全量输出；缺榜单分数的模型不进入该榜帕累托图，但仍保留在额度和单价数据中。
...
|---|---:|---:|
| Code Arena | 140 / 244 | 104 |
| Agent Arena | 144 / 244 | 100 |
| AA Intelligence | 220 / 244 | 24 |
| AA Coding Agent | 79 / 244 | 165 |
| OpenDesign Arena | 72 / 244 | 172 |
| Terminal-Bench 4.0 | 88 / 244 | 156 |
